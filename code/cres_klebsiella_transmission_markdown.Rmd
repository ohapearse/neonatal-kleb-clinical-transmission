...
```{r figure 2, echo = FALSE, fig.dim = c(10, 14), fig.cap = "Figure 2.", warning = FALSE, message = FALSE}

invasive_cluster_snps <- reftran_withinv %>% 
  filter(source.x == "Invasive", snpdist <= 6) %>% 
  group_by(Lane.x) %>% 
  summarise(n = n(), mean = mean(snpdist), median = median(snpdist))
...
figure_two <- ggarrange(snp_invasive, envinvandstoolgraph, nrow = 2, labels = c("A", "B"))
figure_two
```
...
figure_four <- ggarrange(networks, trans_numbers,
           nrow = 2, ncol = 1, labels = c("", "C"),
           heights = c(3, 1))
figure_four

---
title: "Figures for Transmission of extended-spectrum Beta-lactamase Klebsiella pneumoniae on a Malawian neonatal unit: a clinical and genomic analysis of a prospective cohort study"
output:
  word_document:
  html_document:
    df_print: paged
...
reftran_withinv <- read.csv(here("data_markdown", "snp_matrix_clusters.csv"))
reftran <- reftran_withinv %>%
  filter(source.x != "Invasive",
         source.y != "Invasive")
reftran_mgems <- read.csv(here("data_markdown", "snps_data_mgems_updated.csv"))
timediff <- as.matrix(read.csv(here("data_markdown", "timediff_cotnetwork.csv"), 
                     header = FALSE))
neo_all_network_zero <- read.csv(here("data_markdown", "wgs_network_zero_updated.csv"))
neo_all_network_three <- read.csv(here("data_markdown", "wgs_network_three_updated.csv"))
neo_all_network_six <- read.csv(here("data_markdown", "wgs_network_six_updated.csv"))
neo_all_network_twenty <- read.csv(here("data_markdown", "wgs_network_twenty_updated.csv"))
...
neo_all_network_mgems_six <- read.csv(here("data_markdown", "wgs_network_mgems_six_updated.csv"))
neo_all_network_mgems_twenty <- read.csv(here("data_markdown", "mgems_network_twenty.csv"))
invasive_wgs <- read.csv(here("data_markdown", "invasive_stdata_wgs_updated.csv")) %>%
...
envboth <- read.csv(here("data_markdown", "wgs_invasive_stool_updated.csv"))
envboth_stool_msweep <- read.csv(here("data_markdown", "mgems_stool_updated.csv"))
...
#maternal hiv
#mothers are included multiple times in the case of twins or triplets - removing the duplicates
mat <- mat %>%
  distinct(Mat, .keep_all = TRUE)
...
lab_timepoints <- lab_all %>%
  ...
  mutate(Source = case_when(grepl("#", Source) ~ "Ward surfaces",
                             ...
                             Source == "Staff Hand" ~ "Ward surfaces",
                             TRUE ~ Source)) %>%
...
fisher_comparison <- function(source, org1, org2){
  data <- lab_allorganisms %>%
    filter(Source == source,
           organism == org1 | organism == org2)
  fisher <- chisq.test(matrix(c(data$n, data$total - data$n), nrow = 2))
  return(fisher)
}
...
fisher_comparison <- function(source, org1, org2){
  data <- lab_allorganisms %>%
    filter(Source == source,
           organism == org1 | organism == org2) %>%
    arrange(factor(organism, levels = c(org1, org2)))

  
  # Build matrix
  mat <- matrix(c(data$n, data$total - data$n), nrow = 2,
                dimnames = list(
  ...
  prop1 <- data$n[1] / data$total[1]
  prop2 <- data$n[2] / data$total[2]
  
  # Risk difference and 95% CI
  rd <- prop1 - prop2
  se <- sqrt((prop1 * (1 - prop1) / data$total[1]) + 
             (prop2 * (1 - prop2) / data$total[2]))
  lower <- rd - 1.96 * se
  upper <- rd + 1.96 * se
  ...
  result <- list(
    ...
    percentages = data.frame(
      ...
      n = data$n,
      total = data$total,
      percent = round(c(prop1, prop2) * 100, 1)
    ),
    risk_difference = data.frame(
      rd_percent = round(rd * 100, 1),
      lower_95ci = round(lower * 100, 1),
      upper_95ci = round(upper * 100, 1)
    ),
    odds_ratio = round(fisher$estimate, 3),
    or_ci = round(fisher$conf.int, 3),
...
neo_fisher <- fisher_comparison("Child Stool", "kleb", "ecoli")
chit_fisher <- fisher_comparison("Chitenje", "acineto", "kleb")
cot_fisher <- fisher_comparison("Cot", "acineto", "kleb")
hand_fisher <- fisher_comparison("Mother Hand", "acineto", "kleb")
mat_fisher <- fisher_comparison("Mother Stool", "ecoli", "kleb")
surf_fisher <- fisher_comparison("Ward surfaces", "acineto", "kleb")
...
#quantile for the different timepoints 
quantile_random <- quantile(envnull$prop)
quantile_before <- quantile(envbeforeinv$prop)
quantile_after <- quantile(envafterinv$prop)
...
envnull_stool <- envboth %>%
  filter(arm == "null",
         type == "Stool colonisation")
envbeforestool <- envboth %>%
  filter(arm == "test",
         type == "Stool colonisation")
envafterstool <- envboth %>%
  filter(arm == "after",
         type == "Stool colonisation")
...
#iqr for the different timepoints
quantile_randomstool <- quantile(envnull_stool$prop)
quantile_beforestool <- quantile(envbeforestool$prop)
quantile_afterstool <- quantile(envafterstool$prop)
...
#quantile for the different timepoints 
quantile_randomstool_msweep <- quantile(envnull_stool_msweep$prop)
quantile_beforestool_msweep <- quantile(envbeforestool_msweep$prop)
quantile_afterstool_msweep <- quantile(envafterstool_msweep$prop)
...
neo_all_network <- neo_all_network_six
...
network_coef <- data.frame(Factor = c("Edges", "Cots", "Neonatal antibiotics"), 
                           ...
                           CI_high = c(signif(expit(coef((network.model.neoabx)) + summary(network.model.neoabx)$coef[, "Std. Error"])[1], digits = 2),
                                            signif(expit(coef((network.model.neoabx))[1] + coef((network.model.neoabx)) + summary(network.model.neoabx)$coef[, "Std. Error"])[2], digits = 2),
                                            signif(expit(coef((network.model.neoabx))[1] + coef((network.model.neoabx)) + summary(network.model.neoabx)$coef[, "Std. Error"])[3], digits = 2)),
                           CI_low = c(signif(expit(coef((network.model.neoabx)) - summary(network.model.neoabx)$coef[, "Std. Error"])[1], digits = 2),
                                            signif(expit(coef((network.model.neoabx))[1] + coef((network.model.neoabx)) - summary(network.model.neoabx)$coef[, "Std. Error"])[2], digits = 2),
                                            signif(expit(coef((network.model.neoabx))[1] + coef((network.model.neoabx)) - summary(network.model.neoabx)$coef[, "Std. Error"])[3], digits = 2)),
...
#network mgems
neo_mgems <- neo_all_network_mgems_six %>%
...
```{r Figure 1, echo = FALSE, warning = FALSE, fig.dim = c(10, 10), fig.cap =  "Figure 1."}
...
```{r new figure 2, fig.cap = "Figure 2.", echo = FALSE, warning = FALSE, fig.dim = c(14, 14)}
...
```
```{r figure 3, echo = FALSE, message = FALSE, fig.dim = c(10, 14), fig.cap = "Figure 3."}
...
vertex_cols <- c("Neonate's stool" = "yellow2",
...
snpdist_graph <- ggplot(reftran) +
  geom_density(aes(snpdist), lwd = 0.5, colour = "black", fill = "lightgray", alpha = 0.6) +
  theme(text = element_text(size = 20)) +
...
density_panel <- ggarrange(snpdist_graph, density_network_generator,
          nrow = 1, ncol = 2, labels = c("B", "C")
          )

density_panel2 <- ggarrange(contributions_graph,
                            density_panel,
                            density_graph, 
                            nrow = 3, ncol = 1,
                            labels = c("A", "", "D"))
...
```{r figure 4, echo = FALSE, fig.dim = c(10, 14), fig.cap = "Figure 4.", warning = FALSE, message = FALSE}

invasive_cluster_snps <- reftran_withinv %>% 
  filter(source.x == "Invasive", snpdist <= 6) %>% 
  group_by(Lane.x) %>% 
  summarise(n = n(), mean = mean(snpdist), median = median(snpdist), sd = sd(snpdist))

snp_invasive <- ggplot(invasive_wgs) +
  geom_histogram(data = invasive_wgs %>% filter(source != "Invasive"), 
                                                    aes(x = sample_date, y = after_stat(count), colour = ST), alpha = 0, bins = 15) +
  geom_histogram(data = invasive_wgs %>% filter(source == "Invasive"), 
                                                    aes(x = sample_date, y = after_stat(count * 5), fill = ST), colour = "black", alpha = 0.8, bins = 30) +
  ...
  scale_y_continuous(
    ...
    name = "Non-invasive ward isolates",
    # Add a second axis and specify its features
    sec.axis = sec_axis( trans=~./5, 
                         name="Invasive isolates",
...
stgraph_cols <- c("null" = "mediumblue", "test" = "red", "after" = "seagreen")

envinvandstoolgraph <- ggplot(envboth %>% 
                                mutate(arm = fct_relevel(arm, "null", "test", "after"))) +
...
figure_four <- ggarrange(snp_invasive, envinvandstoolgraph, nrow = 2, labels = c("A", "B"))
figure_four
```
...
```{r figure 5, echo = FALSE, warning = FALSE, fig.cap = "Figure 5.", fig.dim = c(14, 14)}
...
figure_five <- ggarrange(networks, trans_numbers,
           nrow = 2, ncol = 1, labels = c("", "C"),
           heights = c(3, 1))
figure_five
...
```{r map, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Supplementary Figure", fig.dim = c(10, 7) }
...
```{r supplementary figure 3, echo = FALSE, fig.cap = "Supplementary Figure", fig.dim = c(10, 10)}
...
```{r snp distance graph, echo = FALSE, message = FALSE, warning = FALSE, fig.dim = c(10, 10), fig.cap = "Supplementary Figure"}
...
```{r sensitivity analysis, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Supplementary Figure", fig.dim = c(10, 13)}
...
```{r density of flux mgems, echo = FALSE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Supplementary Figure", fig.dim = c(10, 13)}
...
```{r sensitivity analysis time, echo = FALSE, warning = FALSE, fig.cap = "Supplementary Figure"}
...
```{r fit metrics, echo = FALSE, message = FALSE, fig.cap = "Supplementary Figure"}
...
```{r quality control, message = FALSE, echo = FALSE, fig.dim = c(7, 10), fig.cap = "Supplementary Figure"}
...
#there are no snplinks at zero snps so the network analysis is not run
...
```{r figures, echo = FALSE, eval = FALSE}

#figure 1
# Open an SVG device
#detach("package:igraph", unload = TRUE)
#library(grid)
#svglite("flowchart_figure.svg", width = 12, height = 16)
#grid.newpage()
#print(flowexclusion_total)
#print(flowexclusion_recruited)
#print(flowexclusion_screenfailure)
#print(flowexclusion_modelling)
#print(flowexclusion_inadequate)
...
ggsave(filename = "cres_model_figure_one_combined.svg", plot = three_micro, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_one_a.svg", plot = all_micro_plot, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_one_b.svg", plot = lolly_kleb_plot, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_one_c.svg", plot = violin_plot, width = 12, height = 16, units = "in")

#figure 2
ggsave(filename = "cres_model_figure_two_combined.svg", plot = figure_two, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_two_a.svg", plot = snp_invasive, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_two_b.svg", plot = envinvandstoolgraph, width = 12, height = 16, units = "in")

#figure 3
ggsave(filename = "cres_model_figure_three_combined.svg", plot = density_panel2, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_three_a.svg", plot = contributions_graph, width = 12, height = 16, units = "in")
ggsave(filename = "cres_model_figure_three_b.svg", plot = snpdist_graph, width = 12, height = 16, units = "in")
svglite("cres_model_figure_three_c.svg", width = 12, height = 16)
plot(density_network, edge.lwd = "prop2", 
                              label = network.vertex.names(density_network), 
                              vertex.cex = 3,
                              label.cex = 1.5,
                              ...
                              vertex.col = vertex_cols)
dev.off()
ggsave(filename = "cres_model_figure_three_d.svg", plot = density_graph, width = 12, height = 16, units = "in")

#figure 4
ggsave(filename = "cres_model_figure_four_combined.svg", plot = figure_four, width = 12, height = 16, units = "in")
svglite("cres_model_figure_four_a.svg", width = 12, height = 16)
plot(refnetwork,
     vertex.shape = "fcircle", 
     vertex.frame.width = 3,
     vertex.size = 6, 
     ...
     edge.width = 2) 
dev.off()
svglite("cres_model_figure_four_b.svg", width = 12, height = 16)
plot(both_network, edge.lwd = "cot", edge.col = "ST", mode = "circle")
legend("topright",
       title = "ST",
       legend=c("ST15", "ST2793", "ST340 & ST15", "ST35", "ST307", "No transmission link"),
       col=c("red", "blue", "gold1", "green", "pink", "snow2"),
       ...
       lwd = 3)
dev.off()
ggsave(filename = "cres_model_figure_four_c.svg", plot = trans_numbers, width = 12, height = 6, units = "in")
